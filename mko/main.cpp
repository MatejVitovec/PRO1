...
double density(const Vector3& w)
{
    return w[RHO];
}


double velocity(const Vector3& w)
{
    return w[RHO_U] / w[RHO];
}


double totalEnergy(const Vector3& w)
{
    return w[RHO_E] / w[RHO];
}


double pressure(const Vector3& w)
{
    return (GAMMA - 1.0)*density(w)*(totalEnergy(w) - 0.5*velocity(w)*velocity(w));
}


double internalEnergy(const Vector3& w)
{
    return pressure(w)/((GAMMA - 1.0)*density(w));
}


double soundSpeed(const Vector3& w)
{
    return sqrt((GAMMA*pressure(w))/density(w));
}


Vector3 flux(const Vector3& w)
{
    double u = velocity(w);
    double p = pressure(w);
...
{
    std::string inputFileName = "cases/sod2.txt";
    std::string outputFileName = "results/sod2HLL.txt";
